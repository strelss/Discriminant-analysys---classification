    ...
    if event == 'Начать исследование':
        ...
        class Diagnostic_assessment:
            ...
            def main(self):
                ...
                # self.probab_mist = self.F1 * self.F4 + self.F2 * self.F3
                self.probab_mist = self.all_var[3] * self.all_var[6] + self.all_var[4] * self.all_var[5] + \
                                   ((self.all_var[1] * self.all_var[2]) / (2.5 * self.all_var[0] * (self.all_var[1] ** 2 - self.all_var[2] ** 2))) * \
                                   (self.all_var[2] * self.all_var[7] * (self.all_var[5] - self.all_var[6]) - self.all_var[1] * self.all_var[8] * (self.all_var[3] - self.all_var[4]))
